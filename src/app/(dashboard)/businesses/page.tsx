...
import { Eye, Search, Plus, ChevronDown, Download, Building2, MapPin } from "lucide-react";
import {
...
import { BackfillProvincesDialog } from "@/components/admin/BackfillProvincesDialog";
...
export default function BusinessesPage() {
  ...
  const [backfillResult, setBackfillResult] = useState<BackfillProvincesResponse | null>(null);
  const [backfillProvinces, { isLoading: isBackfilling }] = useBackfillProvincesMutation();

  const handleBackfillProvinces = async () => {
    try {
      const result = await backfillProvinces().unwrap();
      setBackfillResult(result);
    } catch {
      toast.error("Failed to backfill provinces.");
    }
  };
    ...
    <div className="w-full pt-2">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={handleBackfillProvinces}
            disabled={isBackfilling}
            title="One-time: matches old free-text city/province against the fixed province list. Safe to re-run — already-matched businesses are skipped."
            className="inline-flex items-center justify-center gap-2 rounded-full border border-border px-5 py-2.5 text-sm font-medium text-foreground transition hover:bg-accent disabled:pointer-events-none disabled:opacity-50"
          >
            <MapPin className="size-4" />
            {isBackfilling ? "Backfilling..." : "Backfill Provinces"}
          </button>
          <button
            type="button"
      ...
      {backfillResult ? (
        <BackfillProvincesDialog result={backfillResult} onClose={() => setBackfillResult(null)} />
      ) : null}
    </div>
  );
}
