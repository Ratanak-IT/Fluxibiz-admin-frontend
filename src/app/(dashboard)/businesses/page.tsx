...
export default function BusinessesPage() {
  ...
  return (
    <div className="  ">
      <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8 dark:bg-background">
  ...
  <div className="mt-6 overflow-visible rounded-2xl border border-neutral-200 dark:border-border">
    <table className="w-full text-left">
      <thead className="bg-neutral-50 text-sm  font-semibold text-neutral-800 dark:bg-muted dark:text-foreground">
        <tr>
      ...
      <tbody className="divide-y divide-neutral-100 dark:divide-border">
        {isLoading && (
          <tr>
            <td colSpan={7} className="px-6 py-12 text-center text-sm text-neutral-500 dark:text-muted-foreground">
              Loading businesses...
            </td>
          </tr>
        )}
        ...
        {error && !isLoading && (
          <tr>
            <td colSpan={7} className="px-6 py-12 text-center text-sm text-red-600 dark:text-red-400">
        ...
        {rows.map((business) => (
          <tr key={business.id} className="hover:bg-neutral-50/60 dark:hover:bg-muted/60">
            <td className="px-6 py-4">
              <Link
                href={`/businesses/${business.id}`}
                className="font-medium text-neutral-900 hover:text-green-700 dark:text-foreground dark:hover:text-green-400"
              >
                {business.name}
              </Link>
              <span className="block text-xs text-neutral-400 dark:text-muted-foreground">/{business.slug}</span>
            </td>
            <td className="px-6 py-4 text-sm text-neutral-600 dark:text-muted-foreground">
  ...
  {data && data.totalPages > 1 && (
    <div className="mt-5 flex items-center justify-between text-sm text-neutral-600 dark:text-muted-foreground">
      <span>
      ...
      <div className="flex gap-2">
        <button
          ...
          className="rounded-full border border-neutral-200 px-4 py-2 disabled:opacity-40 dark:border-border"
        >
          Previous
        </button>
        <button
